;; this example was stolen from Rui Ueyama (the C/C++ god) on GitHub
;; https://github.com/rui314/minilisp/blob/master/examples/nqueens.lisp
;;
;; he said the repo is public domain though so it's okay :)

;; define some equivalent forms to Rui's
(defmacro defun (label args . body)
  `(define ,label
     (lambda ,args ,@body)))
(define println print)
(define setcar set-car!)
(define setcdr set-cdr!)
(define eq eq?)
(define t #t)

;; === BELOW THIS LINE IS MOSTLY NOT MINE ===
;; i've rewritten a lot of his code to be compatible with my Lisp but that's it,
;; the logic is his
...
(defun %iota (m n)
  ;; (println `(enter %iota m = ,m n = ,n))
  ;; (define a
  (unless (<= n m)
    (cons m (%iota (+ m 1) n))))
;; (println `(exit %iota m = ,m n = ,n a = ,a))
;; a)
